...
class Tenant {  
    ...
    // New method to check if property exists  
    public function propertyExists($property_id) {  
        $query = "SELECT COUNT(*) FROM properties WHERE id = ?";  
        $stmt = $this->conn->prepare($query);  
        $stmt->execute([$property_id]);  
        return $stmt->fetchColumn() > 0; // Return true if property exists  
    }  
}  
?>